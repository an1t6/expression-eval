...
def waiting_thread(server_socket):
    while True:
        ...
        if task_queue.full():    # 큐가 가득 찼다면 작업 거부 전송
            ...
            logging.info(f"[{get_system_time()}ms] client {client_num}의 작업이 거부되었습니다")
        else:
            ...
            logging.info(f"[{get_system_time()}ms] client {client_num}에서 작업 {expression}을 요청. waiting thread에 할당.")
...
def management_thread():
    while True:
        if not task_queue.empty():
            ...
            task['op_time'] = calculate_time
            logging.info(f"[{get_system_time()+1}ms] wating thread >> management thread {task['expression']}")
            ...
            with calculate_thread_lock:         #threading.lock을 이용해 다른 스레드가 동시에 접근하지 못하게 방지
                ...
                if online_thread:
                    ...
                    logging.info(f"[{get_system_time()+1}ms] management thread >> calculate thread {task['expression']}")
                else:
                    logging.info(f"[{get_system_time()+1}ms] thread is full!")
            ...
            logging.info(f"[{get_system_time()}ms] client {client_num}에 작업 완료 : {task['expression']} = {round(result, 1)}, 수행 시간 : {calculate_time}")
            logging.info(f"[{get_system_time()}ms] client {client_num}의 총 작업 수 : {client_status[client_num]['total_task']}, 총 수행 시간 : {client_status[client_num]['total_time']}ms, 평균 작업 시간 : {round(client_status[client_num]["total_time"]/client_status[client_num]["total_task"],1)}")
    ...
    while True:
        with status['condition']:
            ...
            if task:
                ...
                task['result'] = result
                with task['done']:  
                    task['done'].notify_all() # notify를 이용해 management로 결과를 다시 전송
...
def run_server(host, port):
    ...
    logging.info(f"[{get_system_time()}ms] 서버가 시작되었습니다.")
    ...
    for _ in range(200):
        ...
        run_calculate_thread = threading.Thread(target=calculate_thread, args=(status,))
